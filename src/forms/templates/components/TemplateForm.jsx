...
const TemplateForm = () => {
  ...
  const variableKeys = Object.keys(watch("variables") ?? {});
  ...
  const handleAddVariable = () => {
    ...
    if (currentMessage.length + newVariable.length > 550) { /* ...Swal igual */ return; }
        ...
        <Box sx={{ height: '100%', overflowY: 'auto', pr: 2 }}>
                ...
                render={({ field, fieldState }) => (
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      if (newEmojiCount > 10) { /* ...Swal igual... */ return; }
                      if (newText.length > 550) { /* ...Swal igual... */ return; }
          ...
          <Box sx={{ width: "100%", marginTop: 2, marginBottom: 2, p: 4, border: "1px solid #ddd", borderRadius: 2 }}>
            ...
            <Button
              ...
              startIcon={<AddIcon />}
              onClick={addButton}
              disabled={buttons.length >= MAX_BUTTONS || Object.keys(validationErrors).length > 0}
              sx={{ mt: 3, mb: 3 }}
            >
              Agregar botón
            </Button>
            ...
            <Typography variant="body2" color={buttons.length >= MAX_BUTTONS ? "error" : "text.secondary"} sx={{ mt: 2 }}>
              {buttons.length} / {MAX_BUTTONS} botones agregados
            </Typography>
          </Box>
