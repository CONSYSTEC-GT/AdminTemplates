...
// PLANTILLAS FLOW
export const createTemplateFlowGupshup = async (appId, authCode, templateData, idNombreUsuarioTalkMe, urlTemplatesGS, validateFn) => {
  if (validateFn && !validateFn()) {
    return null;
  }
  ...
  const formattedButtons = buttons.map((button) => {
    const buttonData = { 
      type: button.type, 
      text: button.text || button.title // Mantener compatibilidad con ambos
    };
    
    // Según el tipo de botón, agregar propiedades específicas
    if (button.type === "URL") {
      buttonData.url = button.url;
    } else if (button.type === "PHONE_NUMBER") {
      buttonData.phone_number = button.phoneNumber;
    } else if (button.type === "FLOW") {
      // Propiedades específicas para botones de tipo FLOW
      buttonData.flow_id = button.flow_id;
      buttonData.flow_action = button.flow_action || "NAVIGATE";
...
export const editTemplateFlowGupshup = async (appId, authCode, templateData, idTemplate, idNombreUsuarioTalkMe, urlTemplatesGS, validateFn) => {
  // Validar campos antes de enviar la solicitud
  if (validateFn && !validateFn()) {
    return null; // Detener la ejecución si hay errores
  }
  ...
  // Formatear botones según su tipo (igual que en la creación)
  const formattedButtons = buttons ? buttons.map((button) => {
    const buttonData = { 
      type: button.type, 
      text: button.text || button.title // Mantener compatibilidad con ambos
    };
  ...
  const requestData = {
    ...
    footer: footer || null,
    exampleMedia: mediaId || null,
    buttons: formattedButtons.length > 0 ? formattedButtons : null,
    example: example,
    exampleHeader: exampleHeader || null,
    enableSample: true,
    allowTemplateCategoryChange: false
  };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        await guardarLogArchivos({
          NOMBRE_EVENTO: "PLANTILLAS_FLOW_GUPSHUP_EDICION_ERROR",
          TIPO_LOG: 2, // Error
          URL_PETICION: url,
          PETICION: completeRequestLog,
          RESPUESTA: errorResponse,
      ...
      Swal.fire({
        title: 'Error',
        text: `❌ Error al editar la plantilla: ${errorResponse.message || "Solicitud inválida"}`,
        icon: 'error',
        confirmButtonText: 'Aceptar',
        confirmButtonColor: '#00c3ff'
      });
    ...
    try {
      await guardarLogArchivos({
        NOMBRE_EVENTO: "PLANTILLAS_FLOW_GUPSHUP_EDICION_EXITOSO",
        TIPO_LOG: 1, // Success
        URL_PETICION: url,
        PETICION: completeRequestLog,
        RESPUESTA: result,
    ...
    try {
      await guardarLogArchivos({
        NOMBRE_EVENTO: "PLANTILLAS_FLOW_GUPSHUP_EDICION_EXCEPTION",
        TIPO_LOG: 3, // Exception
        URL_PETICION: url,
        PETICION: completeRequestLog,
        RESPUESTA: { 
